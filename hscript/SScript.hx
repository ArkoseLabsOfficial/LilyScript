...
class SScript {
	...
	public var shareLoadedClassesBetweenScripts:Bool = true;
    public var primaryInstance:CustomClass = null;
    public var primaryClassHandler:CustomClassHandler = null;
    ...
    private function _setupCustomClass():Void {
        if (baseName != null && interp.customClasses.exists(baseName)) {
            primaryClassHandler = interp.customClasses.get(baseName);
            if (shareLoadedClassesBetweenScripts) Interp.globalCustomClasses.set(baseName, primaryClassHandler);
            var args:Array<Dynamic> = constructorArgs != null ? constructorArgs : [];
            primaryInstance = cast primaryClassHandler.hnew(args);
            constructorArgs = null;
            interp.scriptObject = primaryInstance;

            var variableMaps = [interp.variables];
            if (interp.publicVariables != null)
                variableMaps.push(interp.publicVariables);

            for (map in variableMaps) {
                for (k => v in map) {
                    @:privateAccess
                    if (primaryInstance.hasField(k) || primaryInstance.hasStaticField(k)) {
                        try {
                            primaryInstance.hset(k, v);
                        } catch (e:Dynamic) {}
                    }
                }
            }
        } else {
            primaryClassHandler = null;
    ...
    public function set(key:String, obj:Dynamic):SScript {
        ...
        @:privateAccess
        if (primaryInstance != null && (primaryInstance.hasField(key) || primaryInstance.hasStaticField(key))) {
            primaryInstance.hset(key, obj);
        } else if (primaryClassHandler != null && primaryClassHandler.hasField(key)) {
            primaryClassHandler.hset(key, obj);
        }
    ...
    public function get(key:String):Dynamic {
        ...
        @:privateAccess
        if (primaryInstance != null && (primaryInstance.hasField(key) || primaryInstance.hasStaticField(key)))
            return primaryInstance.hget(key);

        if (primaryClassHandler != null && primaryClassHandler.hasField(key))
            return primaryClassHandler.hget(key);

        if (interp.allowPublicVariables && interp.publicVariables != null && interp.publicVariables.exists(key))
            return interp.publicVariables.get(key);

        if (interp.allowStaticVariables && interp.staticVariables != null && interp.staticVariables.exists(key))
    ...
    public function call(func:String, ?args:Array<Dynamic>):FunctionCall {
        ...
        @:privateAccess
        if (primaryInstance != null && (primaryInstance.hasField(func) || primaryInstance.hasStaticField(func))) {
            var oldCaller = caller;
            try {
                var functionField:Dynamic = primaryInstance.call(func, args);
                caller = {
        ...
        if (primaryClassHandler != null && primaryClassHandler.hasField(func)) {
            var fn = primaryClassHandler.hget(func);
            if (fn != null && Reflect.isFunction(fn)) {
                var oldCaller = caller;
                try {
                    var functionField:Dynamic = Reflect.callMethod(null, fn, args);
                    caller = {
                        exceptions: caller.exceptions,
                        calledFunction: func,
                        succeeded: true,
    ...
    function doFile(scriptPath:String):Void {
        ...
        if (scriptPath != null && scriptPath.length > 0) {
            var cleaned = scriptPath.replace('\\', '/').split('/').pop();
